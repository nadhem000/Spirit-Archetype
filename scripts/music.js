...
class MusicPlayer {
	...
	handleSharedMusic(fileName) {
		if (!fileName) return;
		
		caches.open('shared-music-cache').then(cache => {
			cache.match(`/shared-music/${fileName}`).then(response => {
				if (response) {
					response.blob().then(blob => {
						// Create object URL from the blob
						const objectUrl = URL.createObjectURL(blob);
						
						// Extract filename without the timestamp prefix
						const originalName = fileName.replace(/^shared_\d+_/, '');
						
						// Add to playlist
						this.addSharedTrack(objectUrl, originalName);
						
						// Clean up cache
						cache.delete(`/shared-music/${fileName}`);
						
						// Show success message
						showSuccess(`Added "${originalName}" to playlist`);
					});
				}
			});
		});
	...
	// method to handle adding shared tracks
	addSharedTrack(url, name) {
		// Add to tracks array
		...
		// Update playlist modal if open
		if (window.playlistModal && window.playlistModal.modal.classList.contains('SC1-active')) {
...
class PlaylistModal {
    ...
    createTrackElement(trackPath, index) {
		...
		// Check if this is a shared track (blob URL)
		const isSharedTrack = trackPath.startsWith('blob:');
		const trackName = this.musicPlayer.trackNames[index] || 
						 (isSharedTrack ? 'Shared Track' : trackPath.split('/').pop());
		...
		trackDiv.innerHTML = `
			<div class="SC1-track-info">
				...
				${isSharedTrack ? '<div class="SC1-shared-badge">Shared</div>' : ''}
			</div>
